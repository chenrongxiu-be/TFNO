...
def main():
    ...
    # config
    configfile = os.path.join('config', 'config_lstm.ini')
    ...
    config_path = os.path.join('config', f'config_lstm_{title}.ini')
    ...
    for model_idx in range(node_idx[1]-node_idx[0]):
        # Unlike neural operators, each LSTM is trained for a single node only.
        
        ### Build model ###   
        print(f'Build model ({model_idx})...')
        mymodel = Build_model(hidden_size, num_layers).to(device)
        ...
        trainer = Trainer(model_path, device,
                          ...
                          def_dist, node_idx,
                          loss_weight)
        trainer.train(model_idx, mymodel, epoch_num, train_loaders, val_loaders, optimizer, scheduler, batchsize_train, batchsize_val)
...
class Build_model(nn.Module):
    def __init__(self, hidden_size, num_layers):
        ...
        input_size = 30  # vehicle para + previous time series
        output_size = 1  # future time series
        self.lstm = nn.LSTM(input_size, hidden_size, num_layers, batch_first=True)
        self.fc = nn.Linear(hidden_size, output_size)
                
    def forward(self, x_def, x_veh, hn, cn):     
        # concatenate
        x_def = x_def.unsqueeze(-1)  
        x_veh = repeat(x_veh, 'b k -> b t k', t=x_def.shape[1])  
        ...
        return out.squeeze(-1), (hn.detach(), cn.detach())
...
class Trainer:
    ...
    def train(self, model_idx, model, epoch_num, train_loaders, val_loaders, optimizer, scheduler, batchsize_train, batchsize_val):        
        ...
        for ep in range(epoch_num):
            ...
            train_loss = self.batch_loop(model_idx, model, train_loaders, optimizer, self.start_ep+ep+1, n_train_batch, idx='train')
            ...
            with torch.no_grad():
                t3 = default_timer()               
                val_loss = self.batch_loop(model_idx, model, val_loaders, [], self.start_ep+ep+1, n_val_batch, idx='val')
            ...
            if ave_valloss < self.lowest_loss and ave_valloss > 0: 
                ...
                torch.save({'model_state_dict': model.state_dict()}, os.path.join(self.model_path, f'model{str(model_idx).zfill(2)}.pth')) 
    ...
    def batch_loop(self, model_idx, model, dataloaders, optimizer,
                   ep, n_batch, idx):
        loss_log = None
        count_batch = 0
        for _, dataloader in enumerate(dataloaders):
            for i_batch, \
                ...
                for i_iter in range(pred_iters):       
                    ...
                    if i_iter == 0: 
                        (hn, cn) = (None, None)
                    pred_def, (hn, cn) = model(ip_def, ip_veh, hn, cn)
